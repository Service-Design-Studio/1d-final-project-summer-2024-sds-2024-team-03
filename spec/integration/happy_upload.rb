...
describe LogsController do
  it "returns a success response" do
    ...
    puts(response)
    expect(response).to be_successful
    expect(response.body).to include("Completed Subcategory Categorisation")
    expect(response.body).to include("Completed Feedback Categorisation")
  end
end
describe AnalyticsController do
  describe "Gets the final overall sentiment score" do
    it "retrieves sentiment scores" do
      get :get_sentiment_scores, params: { product: 'Cards', source: 'CSS', fromDate: '11/06/2024', toDate: '13/06/2024' }
      expect(response).to be_successful
      
      json_response = JSON.parse(response.body)
      sentiment_scores = json_response.map { |record| record['sentiment_score'].to_f }
      new_average_sentiment_score = (sentiment_scores.sum / sentiment_scores.size).round(1)
      expect(new_average_sentiment_score).not_to eq(4.2)
