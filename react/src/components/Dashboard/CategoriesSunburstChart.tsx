    ...
    fetch(
      ...
      .then((data: Record<string, string>[]) => {
        if (data.length > 0) {
          ...
          >();
          data.forEach(
            ({ product, subcategory, feedback_category, sentiment_score }) => {
              ...
              const avgSentimentScore = avgSentimentScoresMap.get(key)!;
              ...
              if (!productMap.has(product)) {
                productMap.set(product, {
                  category: product,
                  color: `hsl(${feedbackcategoryHashToHue(product)}, 70%, 50%)`,
                  children: [],
                });
              }
          ...
          setAverageSentimentScores(Array.from(avgSentimentScoresMap.values()));
