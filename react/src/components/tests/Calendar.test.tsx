...
describe("Calendar Component", () => {
    ...
    it("should update fromDate when a new date is selected", async () => {
        ...
        expect(await screen.findByLabelText(/From/i)).toHaveAttribute(
            "aria-invalid",
            "false"
        );
    });
    ...
    it("should update toDate when a new date is selected", async () => {
        ...
        expect(await screen.findByLabelText(/To/i)).toHaveAttribute(
            "aria-invalid",
            "false"
        );
    });
    ...
    it("should fetch earliest and latest dates and have correct min and max dates", async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(
            `${urlPrefix}/analytics/get_earliest_latest_dates`
        );

        fireEvent.change(await screen.findByLabelText(/From/i), {
            target: {value: "30-03-2023"},
        });
        expect(await screen.findByLabelText(/From/i)).toHaveAttribute(
            "aria-invalid",
            "true"
        );
        fireEvent.change(await screen.findByLabelText(/To/i), {
            target: {value: "02-09-2024"},
        });
        expect(await screen.findByLabelText(/To/i)).toHaveAttribute(
            "aria-invalid",
            "true"
