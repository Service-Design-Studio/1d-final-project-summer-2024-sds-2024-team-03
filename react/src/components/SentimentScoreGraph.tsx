...
export default function SentimentScoreGraph({
  ...
  const handleProductChange = (
    ...
    setGraphProducts(typeof value === "string" ? value.split(",") : value);
  };
  ...
  const handleSubcategoryChange = (
    ...
    setGraphSubcategories(typeof value === "string" ? value.split(",") : value);
  };
      ...
      fetch(
        ...
        .then((data: Record<string, string>[]) => {
          ...
          if (graphSubcategories.length == 0) {
            const subcategories: string[] = data.map(
          ...
          const filteredData = data.filter(
            (item) =>
              graphProducts.includes(item.product) &&
              graphSubcategories.includes(item.subcategory)
          );
          const filteredDataGroupedByProduct = filteredData.reduce(
            (acc, item) => {
              if (!acc[item.product]) {
                acc[item.product] = [];
              }
          ...
          if (data.length > 0) {
            setSentimentScores(
              Object.entries(filteredDataGroupedByProduct).map(
                ([product, date_sentiment_score]) => {
                  return {
                    id: product,
                    color: "hsl(8, 70%, 50%)",
                    data: date_sentiment_score.map(
            ...
            setSentimentScores([
              {
                id: "all",
                color: "hsl(8, 70%, 50%)",
                data: data.map(({ date, sentiment_score }) => ({
                  x: formatDate(date),
  ...
  return isDetailed ? (
    ...
    >
      <Paper
        sx={{ p: 2, borderRadius: 2, flexDirection: "row" }}
      ...
      >
        ...
        >
          Sentiment vs Time trend for selected Subcategories & Feedback
          Categories
        </Typography>

        <FormControl sx={{ m: 0, width: "20%" }}>
          <InputLabel id="detailed-sentimentscoregraph-filter-product-label">
            Products
          </InputLabel>
          <Select
            ...
            input={
              <OutlinedInput
                id="detailed-sentimentscoregraph-select-product"
                label="product"
              />
            }
        ...
        <Box
          sx={{ display: "flex", gap: 2, mt: 2, width: "100%", height: 200 }}
        >
