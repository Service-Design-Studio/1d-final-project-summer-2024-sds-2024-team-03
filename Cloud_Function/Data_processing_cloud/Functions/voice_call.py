...
def process_voice_call_data(file_path, product, source):
    try:
        ...
        if file_path.lower().endswith('.xls') or file_path.lower().endswith('.xlsx'):
            ...
            publish_message(f"Unsupported file format for {file_path}. Only Excel (.xls, .xlsx) and CSV (.csv) files are supported.","ERROR")
    ...
    for col in data.columns:
        try:
            if data[col].apply(lambda x: isinstance(x, str) and '/' in x).all():
                ...
                break
            elif data[col].apply(lambda x: isinstance(x, str) and '-' in x).all():
                data[col] = data[col].apply(format_date)
                date_column = col
                # cloud_logger.info(f"Date column identified and formatted: {col}")
                break
        except Exception as e:
            print(f"Error processing column '{col}' for date format: {e}")
    
    if date_column:
        data.rename(columns={date_column: 'Date'}, inplace=True)

    if not date_column:
        publish_message("Error: No date column identified. Date column required for processing",'ERROR')
        print("No date column identified.")
        raise ValueError("Date column required for processing")
